...
; CHECK-NEXT:         Domain :=
; CHECK-NEXT:             [n, c, m] -> { Stmt_body1[i0, i1] : 0 <= i0 < n and 0 <= i1 < m and (c < 0 or c > 0) };
; CHECK-NEXT:         Schedule :=
; CHECK-NEXT:             [n, c, m] -> { Stmt_body1[i0, i1] -> [i0, 1, i1] : c < 0 or c > 0 };
; CHECK-NEXT:         ReadAccess :=    [Reduction Type: NONE] [Scalar: 0] [MAPPED]
; CHECK-NEXT:             [n, c, m] -> { Stmt_body1[i0, i1] -> MemRef_A[i0] };
; CHECK-NEXT:         MustWriteAccess :=    [Reduction Type: NONE] [Scalar: 0] [MAPPED]
; CHECK-NEXT:             [n, c, m] -> { Stmt_body1[i0, i1] -> MemRef_A[i0] };
; CHECK-NEXT:     Stmt_body2
; CHECK-NEXT:         Domain :=
; CHECK-NEXT:             [n, c, m] -> { Stmt_body2[i0, i1] : c = 0 and 0 <= i0 < n and 0 <= i1 < m };
; CHECK-NEXT:         Schedule :=
; CHECK-NEXT:             [n, c, m] -> { Stmt_body2[i0, i1] -> [i0, 0, i1] };
; CHECK-NEXT:         ReadAccess :=    [Reduction Type: NONE] [Scalar: 0] [MAPPED]
; CHECK-NEXT:             [n, c, m] -> { Stmt_body2[i0, i1] -> MemRef_A[i0] };
; CHECK-NEXT:         MustWriteAccess :=    [Reduction Type: NONE] [Scalar: 0] [MAPPED]
; CHECK-NEXT:             [n, c, m] -> { Stmt_body2[i0, i1] -> MemRef_A[i0] };
; CHECK-NEXT: }
