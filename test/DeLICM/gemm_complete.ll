; RUN: opt %loadPolly -basicaa -loop-rotate -licm -polly-scops -polly-delicm -analyze < %s
; 
; dgemm kernel
; C := alpha*A*B + beta*C
; C[ni][nj]
...
; CHECK-NEXT:     Mapping of MemRef_beta_c_alpha_a_b_lcssa__phi {
; CHECK-NEXT:         Primary:
...
; CHECK-NEXT:            new: [nj, nk, ni] -> { Stmt_nk_inc[i0, i1, -1 + nk] -> MemRef_C[i0, i1] : nk > 0 and 0 <= i0 < ni and 0 <= i1 < nj; Stmt_nk_inc[0, i1, -1 + nk] -> MemRef_C[0, i1] : nk > 0 and ni <= 0 and 0 <= i1 < nj };
...
; CHECK-NEXT:             Written : [nj, nk, ni] -> { [MemRef_C[i0, i1] -> [i0, i1, 2, -1 + nk, 1{{\]\]}} -> [Stmt_nk_alpha[i0, i1, -1 + nk] -> Val_beta_c_alpha_a_b[{{\]\]}} : nk > 0 and 0 <= i0 < ni and 0 <= i1 < nj; [MemRef_C[0, i1] -> [0, i1, 2, -1 + nk, 1{{\]\]}} -> [Stmt_nk_alpha[0, i1, -1 + nk] -> Val_beta_c_alpha_a_b[{{\]\]}} : nk > 0 and ni <= 0 and 0 <= i1 < nj }
; CHECK-NEXT:     }
; CHECK-NEXT:     Mapping of MemRef_beta_c_alpha_a_b {
; CHECK-NEXT:         Primary:
; CHECK-NEXT:             MustWriteAccess :=    [Reduction Type: NONE] [Scalar: 1]
...
; CHECK-NEXT:     }
; CHECK-NEXT:     Mapping of MemRef_beta_c_alpha_a_b3__phi {
; CHECK-NEXT:         Primary:
; CHECK-NEXT:             ReadAccess :=    [Reduction Type: NONE] [Scalar: 1]
